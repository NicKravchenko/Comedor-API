...
class DishSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        restaurants = Restaurant.objects.all().filter(user=validated_data['user']).order_by('-id')

        if (validated_data['restaurant'] not in restaurants):
            raise serializers.ValidationError({"detail": "Restaurant does not belong to user"})
